...
export const saveSuccess = (grid, columns, savedItems) =>
  (dispatch, getState) => {
    Utils.checkGridParam(grid);
    dispatch({
...
export const savePartialSuccess = (grid, columns, savedItems) =>
  (dispatch, getState) => {
    Utils.checkGridParam(grid);
    dispatch({
          ...
          if (value !== '' || value !== null || value !== undefined) {
            ...
            if (editData) {
              editData.forEach((editDataItem, editDataItemId) => {
                const foundIndex =
                  allData.findIndex(d => d.getIn(grid.idKeyPath) === editDataItemId);
                if (foundIndex !== -1) {
...
export const cellShowMessages = (grid, messages) =>
  (dispatch) => {
    Utils.checkGridParam(grid);
    dispatch({
...
export const cellSelectionChange = (grid, selectedCell = null) =>
  (dispatch) => {
    Utils.checkGridParam(grid);
    dispatch({
      selectedCell,
...
export const setEditData = (grid, data, cellMessages = Map()) =>
  (dispatch) => {
    Utils.checkGridParam(grid);
    dispatch({
      data,
